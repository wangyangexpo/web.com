...
var helpers = {
	...
	getGameInfo:function(v){
		var extend = v.extend;
		var qr,
			ios_qr = extend['ios_qr'],
			android_qr = extend['android_qr'];
		if(ios_qr.value != '' && android_qr.value != ''){
			qr = extend['common_qr'].value;
		}else{
			qr = ios_qr.value != ''?ios_qr.value:android_qr.value;
		}
		return {
			qr:qr,
			android_link:extend['android_url'].value,
			ios_link:extend['ios_url'].value
		}
	},
	...
	staticurl_m: function(v) {
		return configs.staticurl_m + v;	
	},
	devurl_m: function(v) {
