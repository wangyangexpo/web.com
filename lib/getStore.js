...
var getStoreContent = function(req,res,callback){
	console.log('yzl-tag:getStore进入');
	...
	if(req.cookies.sign) {
		...
		//	console.log(data);
			...
			if(_data.msg == 'success') {
				res.cookie('nickname',_data.nickname);
				//res.setHeader("Set-Cookie", ['nickname='+_data.nickname+'']);
				request.get({url: config.storeurl + '/cart/count'}, function (err, httpResponse, body) {
					if (!err && httpResponse.statusCode == 200) {
						callback(body, (req.cookies.nickname==0||req.cookies.nickname=='')?_data.nickname:req.cookies.nickname);
						//callback(body, 'a:'+_data.nickname);
					} else {
						if (httpResponse) {
